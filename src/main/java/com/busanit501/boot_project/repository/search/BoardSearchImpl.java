...
public class BoardSearchImpl extends QuerydslRepositorySupport implements BoardSearch {
    ...
    public Page<BoardListAllDTO> searchWithAll(String[] types, String keyword, Pageable pageable) {
        ...
        //ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ검색조건, 위의 메서드 가져오기.ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
        // 순서4, 옵션
        // where 조건절 추가. 위의 내용 재사용.
        // where 조건절 , BooleanBuilder 를 이용해서 조건 추가.
        // select .. from board where ....
        ...
        JPQLQuery<Tuple> tupleJPQLQuery = boardJPQLQuery.select(board,reply.countDistinct());
            //boardJPQLQuery : 기존 board테이블에 reply 댓글 갯수를 붙여놓은 테이블
        //예) bno title(댓글 갯수포함) writer  regDate ->
        // 이 내용들 boardJPQLQuery 들어가 있다.

        // 순서6
        List<Tuple> tupleList = tupleJPQLQuery.fetch(); //fetch : 데이터 가져오기
        // 엔티티 클래스 -> dto 변환 작업.
        ...
        List<BoardListAllDTO> dtoList = tupleList.stream().map(tuple -> {
            ...
            // BoardImage : 엔티티클래스 -> BoardImageDTOO 처리하기. (형변환)
